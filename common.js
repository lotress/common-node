...
/*eslint no-console: 0*/
var BinaryHeap, M, MinCapacity, None, allAwait, allPromise, apply, bindObject, concatArr, deadline, defaultHeapOptions, delay, eventPromise, expandApply, firstElement, flatArray, flatObject, genLog, genWrap, getCapacity, getIterator, identity, invokeAsync, invokePromises, isFunction, isGenerator, isGeneratorFunction, isIterable, isMultiIterable, isObject, isSymmetry, isType, iterConstructor, logError, logInfo, logLevel, makeFrame, mapArr, mapList, newMessageArray, newPool, pall, pushMap, raceAwait, racePromise, retry, sequence, tco, zipApplyArr;
...
eventPromise = (event, eventReject = null) => {
  return (emitter) => {
    return new Promise((resolve, reject) => {
      if (!emitter) {
        return resolve();
      ...
      if (eventReject) {
        emitter.on(eventReject, (...args) => {
          return reject(emitter, ...args);
        });
      }
      return emitter.on(event, (...args) => {
        return resolve(emitter, ...args);
      });
    });
  };
};
...
module.exports = {identity, None, M, concatArr, flatArray, flatObject, invokeAsync, allAwait, raceAwait, delay, deadline, retry, pall, makeFrame, firstElement, sequence, pushMap, isFunction, isGenerator, tco, BinaryHeap, genWrap, eventPromise, newMessageArray, newPool, logInfo, logError, genLog};
